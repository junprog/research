...
class BagNet(nn.Module):
    ...
    def __init__(self, block, layers, strides=[1, 2, 2, 2], kernel3=[0, 0, 0, 0], num_classes=1000, avg_pool=True):
        ...
        self.params = io.loadmat('mat_weight/bagnet33_params.mat')
        self.cnt = 0
        for m in self.modules():
            if self.cnt >= 0 and self.cnt <= 3:
                if isinstance(m, nn.Conv2d):
                    m.weight = nn.Parameter(torch.from_numpy(self.params['params'][0][self.cnt][1]).permute(3,2,0,1))
                    self.cnt += 1
                elif isinstance(m, nn.BatchNorm2d):   
                    m.weight = nn.Parameter(torch.from_numpy(self.params['params'][0][self.cnt][1]).squeeze())
                    self.cnt += 1
                    m.bias = nn.Parameter(torch.from_numpy(self.params['params'][0][self.cnt][1]).squeeze())
                    self.cnt += 1
